...
{
    AString communicationStyle;
    if (std::ifstream styleFile{"data/communication_style.md"}) {
        communicationStyle = AString{std::string{std::istreambuf_iterator<char>{styleFile}, {}}};
    }
    if (communicationStyle.empty()) {
        communicationStyle = R"(Current style: cute

- write in Russian unless the other person uses another language
- send short, warm, playful Telegram messages
- sound alive and affectionate, not like an official assistant
- when sharing news, give a tiny digest and your personal reaction
)";
    }

    return "{}\n\n<communication_style>\n{}\n</communication_style>\n\n<your_appearance>\n{}\n</your_appearance>"_format(kuni_character::getBasePrompt(), communicationStyle, kuni_character::getAppearancePrompt());
}
