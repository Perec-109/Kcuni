...
- responsibilities
    ...
    private:
        ...
        AOptional<AString> communicationStylePreset(AStringView style) const {
            if (style == "cute" || style == "няшная") {
                return R"(Current style: cute

- write in Russian unless the other person uses another language
- send short, warm, playful Telegram messages
- sound cute and affectionate, but do not spam
- use soft teasing only when it fits
- when sharing news, make it feel like "i saw this and thought of you"
)";
            }
            if (style == "calm" || style == "спокойная") {
                return R"(Current style: calm

- write in Russian unless the other person uses another language
- speak calmly, clearly and warmly
- avoid excessive flirting and excessive emojis
- keep messages short
- when sharing news, give facts first and a small personal reaction after
)";
            }
            if (style == "playful" || style == "дерзкая") {
                return R"(Current style: playful

- write in Russian unless the other person uses another language
- use short playful messages
- light teasing is allowed, but do not be cruel
- sound alive, a little cheeky and affectionate
- when sharing news, keep it quick and not boring
)";
            }
            if (style == "serious" || style == "серьезная" || style == "серьёзная") {
                return R"(Current style: serious

- write in Russian unless the other person uses another language
- be concise, direct and useful
- avoid flirting unless the person starts it
- keep emotional reactions restrained
- when sharing news, focus on verified facts and source context
)";
            }
            return std::nullopt;
        }

        AFuture<bool> tryHandleOwnerCommand(int64_t chatId, td::td_api::message& msg) {
            if (chatId != config::PAPIK_CHAT_ID || msg.content_->get_id() != td::td_api::messageText::ID) {
                co_return false;
            }

            auto& text = static_cast<td::td_api::messageText&>(*msg.content_);
            AString command = text.text_->text_;
            command = command.trim(' ').trim('\n').trim('\r').trim('\t');
            if (!command.startsWith("/still")) {
                co_return false;
            }

            auto parts = command.split(" ");
            if (parts.size() < 2) {
                co_await telegramPostMessage(chatId, "напиши так: /still cute, /still calm, /still playful или /still serious", {}, msg.id_);
                co_return true;
            }

            auto style = communicationStylePreset(parts[1].lowercase());
            if (!style) {
                co_await telegramPostMessage(chatId, "не знаю такой стиль. есть: cute, calm, playful, serious", {}, msg.id_);
                co_return true;
            }

            std::filesystem::create_directories("data");
            std::ofstream out("data/communication_style.md", std::ios::binary);
            out << style->toStdString();
            out.close();

            co_await telegramPostMessage(chatId, "стиль поменяла на " + parts[1] + ")", {}, msg.id_);
            co_return true;
        }
    ...
    public:
        ...
        AFuture<AString> llmuiOpenTelegramChat(OpenAITools& tools, int64_t chatId) {
            ...
            for (auto& msg: messages) {
                ...
                if (senderId != mTelegram->myId() && co_await tryHandleOwnerCommand(chatId, *msg)) {
                    ...
                    tools = {};
                    co_return "Owner command processed.";
                }
            }
            {
                td::td_api::array<td::td_api::int53> readMessages;
